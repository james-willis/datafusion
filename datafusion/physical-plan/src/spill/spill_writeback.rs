...
//!
//! The chunk size can be tuned with the `DATAFUSION_SPILL_WRITEBACK_CHUNK_BYTES`
...
fn chunk_bytes() -> u64 {
    ...
    *CHUNK.get_or_init(|| {
        std::env::var("DATAFUSION_SPILL_WRITEBACK_CHUNK_BYTES")
            .ok()
...
mod tests {
    ...
    fn scopeguard(path: &Path) -> RemoveOnDrop {
        RemoveOnDrop(path.to_path_buf())
    }
